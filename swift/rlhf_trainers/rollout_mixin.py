...
from .utils import (VLLM_LORA_INT_ID, VLLM_LORA_NAME, VLLM_LORA_PATH, FlattenedTensorBucket, TensorLoRARequest,
                    ...
                    phase_timer, profiling_context, profiling_decorator, set_expandable_segments,
...
class RolloutTrainerMixin(RLHFTrainerMixin):
    ...
    def _move_full_model_to_vllm(self):
        ...
        for i, parameter_group in enumerate(self.parameter_groups):
            ...
            with phase_timer(f'  move_full.group[{i}/{len(self.parameter_groups)}]', self), gather_if_zero3(parameters):
                if should_merge:
                    with patch_lora_merge(self.model, parameter_group):
                        self.model.merge_adapter()

                try:
                    with phase_timer(f'    collect_state_dict[{i}]', self):
                        state_dict = self._collect_state_dict_for_vllm(parameter_group, parameter_group_no_lora)
                    with phase_timer(f'    load_state_dict_to_vllm[{i}]', self):
    ...
    def _fast_infer(self, inputs: DataType) -> DataType:
        ...
        with context():
            ...
            if (self.vllm_mode == 'colocate' and self.engine.inner_model_executor.is_sleeping
                ...
                with phase_timer('vllm.wake_up(kv_cache)+empty_cache', self):
                    aggressive_empty_cache()
                    set_expandable_segments(False)
                    self.engine.engine.wake_up(tags=['kv_cache'])

            with phase_timer('rollout.generate', self):
            ...
            if self.vllm_mode == 'colocate' and args.sleep_level > 0:
                with phase_timer('vllm.sleep+empty_cache', self):
                    self.engine.engine.reset_prefix_cache()
