...
| transformers | 5.3.0 | 与 GPU GKD docker 对齐；Qwen3.5 LatentMoE 需 >=5.2.0；在 vllm-ascend 之后安装以覆盖其旧版 pin |
| deepspeed | 0.18.8 | 与 GPU GKD docker 对齐；作为 fallback 安装，Megatron 路径本身不需要 |
| accelerate | 1.13.0 | 与 GPU GKD docker 对齐 |
| peft | 0.18.1 | 与 GPU GKD docker 对齐 |
| safetensors | 0.7.0 | 与 GPU GKD docker 对齐 |
| qwen_vl_utils | >=0.0.14 | 与 GPU GKD docker 对齐 |
| torchao | （卸载） | vllm 安装的 dev 版引用 torch 2.9.0+ 中不存在的算子，安装后立即 `pip uninstall -y torchao` |
| flash-attn / flash-linear-attention / causal-conv1d / liger_kernel | — | CUDA/Triton-only，NPU 不安装；Megatron `--attention_backend flash` 走 MindSpeed 的 `npu_fusion_attention` |
