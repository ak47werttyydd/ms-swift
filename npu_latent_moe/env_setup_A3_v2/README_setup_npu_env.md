...
| deepspeed | 0.18.8 | 与 GPU GKD docker 对齐；student 训练用 ZeRO-3 |
...
| flash-attn / flash-linear-attention / causal-conv1d / liger_kernel | — | CUDA/Triton-only，NPU 不安装；attention 走 `--attn_impl sdpa`（torch-npu `npu_fusion_attention`） |
