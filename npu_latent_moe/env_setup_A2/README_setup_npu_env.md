...
| 项目 | 要求 |
|------|------|
| 操作系统 | openEuler（或其他 Linux） |
| CPU 架构 | aarch64 |
| NPU | Ascend 910B1（或同系列 910B/C） |
| NPU 显存 | 910B1 每卡 32 GB；Qwen3.5-35B-A3B teacher 需 TP4（4 卡 × 32 GB） |
...
1. CANN 8.5.1 已通过三个 `.run` 包安装（toolkit + 910b-ops + nnal），并能 source `set_env.sh`：
   ...
   /usr/local/Ascend/nnal/atb/set_env.sh   # 可选，libatb.so
   ```
...
| `CANN_SETENV` | `/usr/local/Ascend/ascend-toolkit/set_env.sh` | CANN 环境脚本路径 |
| `NNAL_SETENV` | `/usr/local/Ascend/nnal/atb/set_env.sh` | NNAL 环境脚本路径（可选） |
