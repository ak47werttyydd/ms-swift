...
# ═══════════════════════════════════════════════════════════════════════════════
# GKD 24-layer Rezaul LatentMoE — continue from checkpoint-900
#
# Scenario A+B (mixed):
#   A: Aux load enabled (output_router_logits=true, coef=0.01 in config.json)
#      num_experts_per_tok: 8→16 in config.json
#      Fresh LR warmup (50 steps)
#   B: Freeze attn + GDN layers, train MoE only (~35% faster per step)
#
# PREREQ: checkpoint-900-aux-freezeattn/config.json must already have:
#   text_config.num_experts_per_tok: 16
#   text_config.output_router_logits: true
#   text_config.router_aux_loss_coef: 0.01
#
# Usage:
#   bash gkd_24l_latentMoE_start900ckpt_aux_freezeattn.sh
#   MAX_STEPS=500 bash gkd_24l_latentMoE_start900ckpt_aux_freezeattn.sh
# ═══════════════════════════════════════════════════════════════════════════════
...
STUDENT_24L="${STUDENT_24L:-/home/a84400789/ms-swift/output/gkd_rezaul_latentmoe_24l/v1-20260418-125431/checkpoint-900-aux-freezeattn}"
GKD_PLUGIN="${SCRIPT_DIR}/qwen35_latentmoe/gkd_plugin.py"

OUTPUT_24L="${OUTPUT_24L:-${SCRIPT_DIR}/output/gkd_rezaul_latentmoe_24l_from900_top16_aux_freezeattn}"

# ── Max steps (from step 900 onwards) ───────────────────────────────────────
...
echo "=== 24-layer GKD from ckpt-900 — Scenario A+B (aux loss + freeze attn/GDN, top-16, max_steps=${MAX_STEPS}) ==="
